...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        #    and continue working on the problem.
        # --------------------------------------------------------------
        s1 = self.contents + additional_contents
        s = ''
        if len(s1) > self.volume:
            for k in range(self.volume - len(self.contents)):
                self.contents = self.contents + additional_contents[k]
            for k in range(len(s1) - self.volume):
                s = s + s1[self.volume + k]
        else:
    ...
    def reset(self):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # --------------------------------------------------------------
        self.brc = self.brc + [self.contents]
        b1 = Box(self.oc, self.ov)
        self.volume = b1.volume
        self.contents = b1.contents
    ...
    def combined_box(self, other_box):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # --------------------------------------------------------------
        b1 = Box(self.contents + other_box.contents, self.volume + other_box.volume)
        return b1
